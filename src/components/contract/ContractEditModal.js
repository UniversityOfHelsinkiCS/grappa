...
import ContractFormElement from './ContractFormElement';
...
class ContractEditModal extends Component {
    ...
    generateFormFields = () => {
        var elements = this.parseContractData(this.state.editedFormData).map((element) =>
    ...
    createFormField = (c) => {
        return (
            <ContractFormElement key={ c.fieldName } fieldName={ c.fieldName } content={ c.content } originalContent={ c.originalContent } textField={ c.textField } onChange={ this.onFieldChange }/>
        );
    }
